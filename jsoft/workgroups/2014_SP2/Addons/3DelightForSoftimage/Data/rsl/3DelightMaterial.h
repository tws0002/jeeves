		...
		output color o_reflection_environment;)
{
	...
	{
		getReflectionEnvironmentMapParameters( envmap, envspace );
	}
		...
		output color o_outTransparency;)
{
...
#else
			indirect_diffuse =
				getIndirectDiffuse( Nf_base, indirect_diffuse_weight );
#endif
		}
		else
		{
		...
		isoutput( "aov_indirect" )!=0 )
	{
		if( isoutput( "aov_occlusion" ) )
		{
			outputchannel(
					"aov_occlusion",
					gi_visibility * opacity );
		}
		if( isoutput( "aov_env_diffuse" ) )
		{
			outputchannel(
					"aov_env_diffuse",
					gi_env_diffuse * diffuse_color * coating_absorbed );
		}
		if( isoutput( "aov_indirect" ) )
		{
			outputchannel(
					"aov_indirect",
					gi_color_bleeding * diffuse_color * coating_absorbed );
		}
	}
	...
	if( isoutput( "aov_rt_reflection" ) )
	{
		outputchannel(
				"aov_rt_reflection",
				coating_reflect_object * coating_color +
				reflect_object * reflect_color * coating_absorbed );
	}
	if( isoutput( "aov_env_reflection" ) )
	{
		outputchannel(
				"aov_env_reflection",
				coating_reflect_environment * coating_color +
				reflect_environment * reflect_color * coating_absorbed );
	}
...
/* XSI7 coating on, sss on */
void _3DFS_3DelightMaterial(
		/* Coating parameters */
		float i_coating_on;
		RGBA_DECLARE( i_coating_color );
		RGBA_DECLARE( i_coating_transmittance );
		float i_coating_thickness;
		float i_coating_roughness;
		float i_coating_samples;
		float i_coating_ior;
		...
		/* Reflection parameters */
		RGBA_DECLARE( i_reflect_color );
		float i_reflect_roughness;
		float i_reflect_samples;
		float i_reflect_ior;
		float i_reflect_anisotropy;
		RGBA_DECLARE( i_reflect_anisotropy_direction );
		point i_reflect_anisotropy_space;
		float i_reflect_has_reflection;
		float i_reflect_has_specular;
		float i_reflect_has_environment;
		...
		/* Subsurface scattering parameters */
		float i_sss_on;
		RGBA_DECLARE( i_sss_scattering );
		float i_sss_scattering_scale;
		RGBA_DECLARE( i_sss_transmittance );
		float i_sss_transmittance_scale;
		float i_sss_ior;
		float i_sss_scale;
		string i_sss_group;

		RGBA_DECLARE( i_refract_color );
		float i_refract_roughness;
		float i_refract_samples;
		float i_refract_ior;
		...
		RGBA_DECLARE_OUTPUT( o_outColor ) )
{
	_3DFS_3DelightMaterial(
		/* Coating parameters */
		i_coating_on,
		i_coating_color,
		i_coating_transmittance,
		...
		RGBA_DECLARE_OUTPUT( o_outColor ) )
{
	_3DFS_3DelightMaterial(
		/* Coating parameters */
		i_coating_on,
		color(1, 1, 1)    /* coating_color */ ,
		color(1, 0.5, 0.1)    /* coating_transmittance */ ,
		0.01    /* coating_thickness */ ,
		0    /* coating_roughness */ ,
		8    /* coating_samples */ ,
		1.3    /* coating_ior */ ,
		true    /* coating_has_reflection */ ,
		true    /* coating_has_specular */ ,
		true    /* coating_has_environment */ ,
		...
		RGBA_DECLARE_OUTPUT( o_outColor ) )
{
	_3DFS_3DelightMaterial(
		...
		/* Subsurface scattering parameters */
		i_sss_on,
		color(0.73, 0.873, 1)    /* sss_scattering */ ,
		3    /* sss_scattering_scale */ ,
		color(0.79, 0.59, 0.29)    /* sss_transmittance */ ,
		0.01    /* sss_transmittance_scale */ ,
		1.6    /* sss_ior */ ,
		0.1    /* sss_scale */ ,
		""    /* sss_group */ ,
