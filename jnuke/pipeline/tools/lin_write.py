...
'''
print '> importing jnuke.pipeline.tools.lin_write'
import nuke, os
import core
import core.locate
...
def run():
    ...
    count = 1
    while nuke.exists('Linear_Write_' + str(count)):
        count += 1
    w.knob('name').setValue('Linear_Write_' + str(count))
    
    t = nuke.Tab_Knob("Linear Write")
    w.addKnob(t)
    feedback = """
    [value dirName]
    """
